...
def make_frames_target_step(
    ...
    **kwargs: Any,
    ...
    and ``kwargs``; that class's own ``__post_init__`` unconditionally
    deep-freezes ``kwargs`` at construction, not an anonymous closure -- see
    the module docstring and that class's own docstring for why this is the
    E4 authenticity seam. ``config`` is built from the *same* frozen
    snapshot ``fn.kwargs`` holds (plus the descriptive ``"target"`` label),
    so introspecting ``config`` and executing ``fn`` always agree.
    """
    fn = _resolve_target(target)
    call = BoundFramesTargetCall(fn, kwargs)
