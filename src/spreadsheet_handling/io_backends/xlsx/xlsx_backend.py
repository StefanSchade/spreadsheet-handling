...
class ExcelBackend(BackendBase):
    """
    XLSX adapter using the IR rendering pipeline (compose → passes → plan → render).
    """
    ...
    def read_multi(
        ...
        """
        Read all visible sheets from an XLSX file via IR-based extraction.
...
def write_xlsx(
    ...
    Delegates to ExcelBackend().write_multi.
    """
