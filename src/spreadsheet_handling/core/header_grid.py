"""Backend-neutral helpers for the exact multi-row header grid.
...
consumed by the render-plan writer (GX-3a) and the grouped-XRef reverse
reconstruction step (GX-3b); this module holds only pure, format-neutral
operations on it.
...
This lives in ``core`` (leaf, no inward dependencies) so both ``rendering``
(GX-3a write path) and ``domain`` (GX-3b reverse reconstruction) can use it
without crossing a layer boundary; ``rendering.header_grid`` re-exports it for the
established GX-3a import path.
"""
