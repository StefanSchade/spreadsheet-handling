...
def _get_pipeline_specs_or_die(
    pipelines: Dict[str, Any], name: str, profile: str | None
) -> list[dict]:
    ...
    raise SystemExit(f"Unknown pipeline '{name}'. Available: {list(pipelines)}")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # yaml config options
    parser.add_argument(
        "--config", help="Path to a config YAML. May include io, pipelines, pipeline."
    )
    parser.add_argument(
        "--pipeline", help="Name of a pipeline defined under 'pipelines:' in --config."
    )
    parser.add_argument(
        "--steps", help="Path to a YAML defining steps for ad hoc run. May also include io"
    )
    parser.add_argument(
        "--profile",
        help="Name of 'io.profiles[...]' in --config (binds IO and optional default pipeline).",
    )
    ...
    parser.add_argument(
        "--in-kind", help="Override input.kind (e.g., json_dir, yaml_dir, xlsx, ods, calc)"
    )
    parser.add_argument("--in-path", help="Override input.path")
    parser.add_argument(
        "--out-kind", help="Override output.kind (e.g., json_dir, yaml_dir, xlsx, ods, calc)"
    )
...
def cli_entry() -> None:
    ...
    Routes ``sheets-run`` through :func:`run_cli` so the installed entry
    point benefits from consistent error UX (short `Error: ...` line,
    `Interrupted by user.` on Ctrl+C, traceback in `--debug` / `-vv`,
    exit code 130 for interrupts). Tests continue to import :func:`main`
    directly to assert return codes and exception propagation.
    """
    run_cli(main)
