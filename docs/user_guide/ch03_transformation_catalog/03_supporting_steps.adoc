...
rendered. The `frame:` field names the logical or projected frame whose column
roles are resolved. `frame:` may be omitted when the visible sheet and logical
frame are unambiguous, but keeping it explicit is recommended for consumer
pipelines that use projected workbook views.
